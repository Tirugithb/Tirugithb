# Hi, I'm Tiru 

## Technical Lead | Senior Test Automation Engineer | AI Automation Engineer

Experienced professional with 9+ years of expertise in Test Automation, ETL Testing, Data Warehouse Testing, Database Testing, BI Testing, Salesforce CRM Analytics, AI-Assisted Automation, Mobile Automation, Web Automation, and Enterprise Healthcare Systems.

### Core Skills

* Python Automation
* Appium Mobile Automation
* Playwright Web Automation
* Selenium Automation
* AI Agents & Generative AI
* ETL Testing
* Data Warehouse (DWH) Testing
* Database Testing
* BI & Dashboard Validation Testing
* Salesforce CRM Analytics (Tableau CRM / Einstein Analytics)
* AWS Testing
* SQL Server
* Oracle
* Informatica
* SSIS
* Data Analytics

### Automation Expertise

* Android Mobile Automation
* Samsung Device Automation
* Tablet Health Monitoring
* Google Play System Update Automation
* Framework Development
* Reporting Automation
* Sequential Execution Frameworks

### AI Projects

* AI Analytical Agent
* AI Personal Assistant
* Custom ChatGPT Application
* Repository Error Analysis Agent
* Infrastructure Chat Agent
* Intelligent Automation Solutions

### Domain Expertise

* Healthcare Systems (Medtronic)
* Samsung/HARMAN Device Ecosystem
* Enterprise Reporting & Analytics
* Data Integration & Validation

### Featured Repositories

* Android Device Automation Framework
* Playwright Web Automation Framework
* Selenium Web Automation Framework
* AI Analytical Agent
* Custom ChatGPT
* Repository Error Agent
* Tablet Health Monitor

### Certifications

* AWS Certified Solutions Architect
* Microsoft Azure Fundamentals
* DevOps Fundamentals
* Data Analyst Certification
* Automation Anywhere Advanced RPA Professional

### Connect With Me

* GitHub: https://github.com/Tirugithb
* LinkedIn: Add your LinkedIn profile URL
